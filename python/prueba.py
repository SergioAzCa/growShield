        ...
        while font.size > 0:
            ...
            if len(lines) <= max_lines and len(words) == 0:
                ...
                for line in lines:
                    ...
                    self._draw.text(
                        (x, y), line, font=self.font, fill=textcolor)
                    y += line_height
...
class MainView(View):
    ...
    def render_channel(self, channel):
        ...
        if active:
            ...
            self._draw.rectangle(
                (x, int((1.0 - saturation) * DISPLAY_HEIGHT),
                 x + bar_width - 1, DISPLAY_HEIGHT),
                channel.indicator_color(
                    saturation) if active else (229, 229, 229),
            )

        y = int((1.0 - warn_level) * DISPLAY_HEIGHT)
        self._draw.rectangle(
            (x, y, x + bar_width - 1, y), (255,
                                           0, 0) if channel.alarm else (0, 0, 0)
        )

        # Channel selection icons
        x += (bar_width - label_width) // 2

        self.icon(icon_channel, (x, label_y), (200, 200, 200)
                  if active else (64, 64, 64))
    ...
    def render(self):
        ...
        self.icon(icon_backdrop.rotate(180),
                  (DISPLAY_WIDTH - 26, 0), COLOR_WHITE)
...
class EditView(View):
    ...
    def render(self):
        ...
        if self._change_mode:
            ...
            self.label("Y", "Change", textcolor=COLOR_BLACK,
                       bgcolor=COLOR_WHITE)

        self._draw.text((3, 36), f"{title} : {text}",
                        font=self.font, fill=COLOR_WHITE)
...
class DetailView(ChannelView):
    ...
    def render(self):
        ...
        if self.channel.enabled:
            ...
            self._draw.rectangle(
                (graph_x, graph_y, graph_x + graph_width, graph_y + graph_height), (50, 50, 50))
            ...
            for x, value in enumerate(self.channel.sensor.history[:graph_width]):
                ...
                self._draw.rectangle(
                    (x, graph_y + graph_height - h, x + 1, graph_y + graph_height), color)

            alarm_line = int(self.channel.warn_level * graph_height)
            r = 255
            if self.channel.alarm:
                r = int(
                    ((math.sin(time.time() * 3 * math.pi) + 1.0) / 2.0) * 128) + 127
...
class Channel:
    ...
    def indicator_color(self, value):
        ...
        r, g, b = [int(((self.colors[b][i] - self.colors[a][i])
                        * blend) + self.colors[a][i]) for i in range(3)]
    ...
    def update_from_yml(self, config):
        if config is not None:
            ...
            self.watering_delay = config.get(
        ...
        if settings_file.is_file():
            try:
                ...
                raise yaml.parser.ParserError(
                    "Error parsing settings file: {} ({})".format(
                        settings_file, e)
                )
...
def main():
    ...
    def connectionBBDD(moisture, canal):

        try:
            now = datetime.now()
            datatime_now = now.strftime("%d/%m/%Y %H:%M:%S")
            conn = psycopg2.connect(
                "dbname='growshield' user='pi' host='192.168.0.48' password='postgres'")
            cursor = conn.cursor()
        except:
        ...
        try:
            ...
            if canal == 1:
                cursor.execute("INSERT INTO ""dataseed1"".""seed"" (soil,temperature,typeGrow,date,data_raw)  VALUES (%s, %s, %s,%s,%s)",
                               (moisture, 0, 'Bonasi1', datatime_now, str("{}")))
            if canal == 2:
                cursor.execute("INSERT INTO ""dataseed1"".""seed"" (soil,temperature,typeGrow,date,data_raw)  VALUES (%s, %s, %s,%s,%s)",
                               (moisture, 0, 'Aguacate2', datatime_now, str("{}")))
            if canal == 3:
                cursor.execute("INSERT INTO ""dataseed1"".""seed"" (soil,temperature,typeGrow,date,data_raw)  VALUES (%s, %s, %s,%s,%s)",
                               (moisture, 0, 'Bonsai2', datatime_now, str("{}")))
        except ValueError:
    ...
    while True:
        for channel in channels:
            ...
            if channel.channel == 1:
                if contador == 4000:
                    connectionBBDD(channel.moisture, channel.channel)
